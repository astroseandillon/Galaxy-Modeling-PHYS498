...
# returns positions for point masses, point masses with respect to their central
# masses, and the positions of the central masses
def positions(ngal,n,nt,solution,extract_data=False,gal_num=0):
    r_cent = np.zeros((ngal,3,nt))
    r = np.zeros((n,3,nt))
    r_com = np.zeros((n,nt))
    for i in range(nt):
        # get central mass velocities
        for j in range(ngal):
            r_cent[j,:,i] = solution[j*2,:,i]
        # get point mass velocities
        r[:,:,i] = solution[ngal*2::2,:,i]
    ...
    for i in range(n):
        ...
        if count % (n//ngal) == 0:
            gal += 1
        r_temp = r[i] - r_cent[gal]
        # rms velocities
        r_com[i] = np.sum(r_temp**2, axis=0)**0.5
        count += 1
    if extract_data:
        r = ExtractGalaxyData(r,ngal,gal_num)
        r_com = ExtractGalaxyData(r_com,ngal,gal_num)
        r_cent = r_cent[gal_num-1]
    return r, r_com, r_cent
...
# get positions or velocities data for a specific galaxy
def ExtractGalaxyData(data, ngals, gal_num):
    n = len(data)
    start = int(n*((gal_num - 1)/ngals))
    stop = int(n*(gal_num/ngals))
...
#-------------------------- plotting function
def galaxyHistogram(v_initial, v_final, n,
                    title1, xlabel, ylabel, label_i, label_f, binw=0.2,
                    legend_loc='upper right',save=False,save_name=''):
    ...
    binStart = np.min([v_initial, v_final]) - binw
    binStop = np.max([v_initial, v_final]) + binw
    bins = np.arange(binStart, binStop, binw)
    fig, ax=plt.subplots(1,1, figsize=(8,4))
    weights = np.ones(n)/n

    ax.hist(v_initial, bins, label= label_i, color="red", weights=weights, linewidth=1.3, alpha=0.5)
    ax.hist(v_final, bins, label= label_f, color="black", weights=weights, linewidth=1.3, alpha=0.5)
    
    ax.grid()
    ax.legend(bbox_to_anchor=(1.05, 1), loc=legend_loc, shadow=True)
...
#-------------------------- saving data
def column(tableName, data, columnName):
    c = Column(data= data, name= columnName)
    tableName.add_column(c)
