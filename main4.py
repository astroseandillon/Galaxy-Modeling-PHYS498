...
Parameters:
    ...
    Other parameters:
        ...
        save ; saves solution and initial conditions in a csv file
...
def main(g=4*np.pi**2, m=1, num_galaxies=1, galaxy_pos=np.array([[0,0,0]]),
    ...
    def SaveData(sol, file_name):
        if file_name == '':
            file_name = FileName('data', dir_name)
        # initialize writer
        file = open(dir_name + '/' + file_name, 'w', newline = '')
        data_writer = csv.writer(file, delimiter = ',')
        # save parameters
        param_names = ['g','m','num_galaxies','galaxy_pos','galaxy_vel',
                       'euler_angles','r_outer','n_inner','n_outer',
                       'num_rings','t_min','t_max','nt','grav_gal']
        params = [g,m,num_galaxies,galaxy_pos,galaxy_vel,euler_angles,r_outer,
                  n_inner,n_outer,num_rings,t_min,t_max,nt,grav_gal]
        data_writer.writerow(param_names)
        data_writer.writerow(params)
        # save data
        for it in range(np.shape(sol)[2]):
            data_writer.writerow(' ')
            data_writer.writerow([it])
            for i in range(np.shape(sol)[0]):
                data_writer.writerow(sol[i,:,it])
        file.close()
